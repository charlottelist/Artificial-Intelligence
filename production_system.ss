...
(define rules 
  '(
    (r1 (if (current x) (adjacent y) (not visited y) (not obstacle y))
        (delete (current x))
        (add (path x y) (visited y) (delete_adjacents) (move_to y)))

    (r7 (if (current x) (adjacent y) (not visited y) (obstacle y) (stable y) (height y low))
        (delete (current x))
        (add (path x y) (visited y) (delete_adjacents) (move_to y)))

    (r2 (if (current x) (adjacent y) (path y x) (not obstacle y))
        (delete (current x) (path y x))
        (add (visited x) (delete_adjacents) (backtrack_to y)))

    (r8 (if (current x) (adjacent y) (path y x) (obstacle y) (stable y) (height y low))
        (delete (current x) (path y x))
        (add (visited x) (delete_adjacents) (backtrack_to y)))
